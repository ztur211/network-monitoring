...
### Keep AI chat resilient when best-effort Redis paths fail (this commit, 2026-05-31)

Two best-effort Redis touch-points in the chat flow could take down — or mislabel — a request the AI graceful-degradation design (CLAUDE.md Option 2) is meant to survive:

1. **History read outside the fallback.** `AiService.sendMessageStream` reads `conversation.getHistory` in a `Promise.all` *before* the `try` that wraps the adapter call, and `getHistory` did a bare `await this.redis.get(...)` (only its `JSON.parse` was guarded). A transient Redis error on the read therefore threw past the graceful fallback and 500'd the whole chat request even when the AI provider was healthy. `getHistory` now wraps the `redis.get` in try/catch and returns `[]` on error (mirroring its existing parse-miss behavior) — history is a best-effort context source, so a blip degrades to empty history. Added a `Logger` so the read failure is observable (the benign empty/parse-miss cases stay silent).

2. **Post-stream accounting relabeling a delivered answer.** After `adapter.stream` succeeded and the answer had already streamed to the client via `onToken`, the `appendMessages` + `incrementUsage` + `getUsageCounts` writes ran *inside* the stream `try`. A Redis blip on any of them was caught by the same `catch` and returned the **provider-unavailable fallback envelope** (`providerStatus: 'unavailable'`, `tokensUsed: 0`) — contradicting the real answer the user just watched stream in. Extracted `recordUsageBestEffort`, which swallows accounting errors (logs + returns zeroed usage) so a successful stream always yields a **success** envelope with the real `tokensUsed`. The adapter-failure path is unchanged: if `adapter.stream` itself throws, the graceful fallback still fires (and history/usage are still not written).

Verification: `conversation.service.spec` gains "returns empty history when the Redis read fails"; `ai.service.spec` gains "returns a success envelope (not the fallback) when post-stream accounting fails" (asserts `providerStatus: 'ok'` and the real `tokensUsed`); the existing "adapter throws → fallback, no history/usage written" case still passes (**ai + conversation: 2 suites / 20 tests** green). `tsc --noEmit` clean for the api workspace. Code-only — no migration, no API-contract change (behavior only changes under a Redis fault, where it now degrades instead of 500-ing or mislabeling). Local-only follow-up (gitignored per CLAUDE.md): none.

### Guard the push-scheduler lock acquisition against Redis errors (this commit, 2026-05-31)

`RealtimeGateway.runPushScheduler` is fired as `void this.runPushScheduler()` from a `setInterval` (no `await`), and there is no global `unhandledRejection` handler. It wrapped only the *work* (`pushLatestMetricsToConnectedUsers`) in try/catch — the lock acquisition `await this.redis.set(LOCK, '1', 'EX', ttl, 'NX')` sat *outside* it. So a transient Redis error during lock acquisition became an **unhandled promise rejection** and a silently-dropped push cycle, with nothing logged. This is best-effort background work; it must never throw out of the tick.

Moved the lock `redis.set` inside the try/catch (wrapping the whole cycle). A Redis blip on lock acquisition is now logged at `error` and the tick is simply skipped — the next interval retries. No behavior change on the happy path (lock acquired → push; lock held by another instance → return early).

Verification: `realtime.service.spec` gains a `runPushScheduler` block — a rejecting `redis.set` resolves without throwing and logs (and never reaches `getLatestMetrics`); a `null` lock result skips the cycle; an acquired lock proceeds to `fetchSockets` (**1 suite / 24 tests** green). `tsc --noEmit` clean for the api workspace. Code-only — no migration. Local-only follow-up (gitignored per CLAUDE.md): none.

### Cover the OpenAI-compatible adapter (this commit, 2026-05-31)

Test-only. `OpenAICompatibleAdapter` (selected when `AI_PROVIDER` points at an OpenAI-style endpoint, e.g. a local Ollama) was the only AI adapter with non-trivial branching and had zero unit coverage — its `complete` response mapping and its hand-rolled SSE `stream` parser were only ever exercised behind a live HTTP call. New `openai-compatible.adapter.spec.ts` mocks global `fetch`: `complete` maps a normal response, throws on a non-OK status, and returns empty content on empty `choices`; `stream` accumulates tokens across SSE `data:` chunks while skipping the `[DONE]` sentinel and malformed JSON lines (asserting it does **not** throw on a bad line), approximates the token counts (`len/4`), and throws when the response is not OK or the body is null. Throwing on a non-OK/bodyless response is the correct contract — `AiService` catches it and returns the documented graceful fallback. The `adapter` testRegex entry (added in the prior batch for `NominatimAdapter`) already discovers the spec.

Verification: `openai-compatible.adapter.spec` green (**1 suite / 6 tests**); `tsc --noEmit` clean for the api workspace. No production-code change. Local-only follow-up (gitignored per CLAUDE.md): none.

### Prune dead type/decorator surface (this commit, 2026-05-31)

Removed four exported symbols with zero references anywhere in `apps/` or `packages/` (verified by grep — the only other hits were generated `dist/*.d.ts`):
- `CurrentSession` param decorator (`auth/decorators/current-session.decorator.ts`, whole file) — the codebase uses `@CurrentUser()` everywhere; `request.session` is never read through a decorator.
- `ContextProvider` interface (`ai/context/context-provider.interface.ts`) — no provider `implements` it; `ContextBuilderService` injects the four concrete providers by their DI symbol (the four `*_CONTEXT_PROVIDER` consts in the same file stay).
- `DataSourceCollector` interface (`data-sources/data-sources.interface.ts`) — nothing implements it; its sibling interfaces (`MetricRecord`, `DataSourceStatus`, `RawMetricPayload`) are used and stay.
- `WsEventName` type (`packages/shared/.../realtime.types.ts`) — derived from the heavily-used `WS_EVENTS` const but itself never consumed.

Separately **adopted** (rather than deleted) the previously-unused `ApiError` shared type: `GlobalExceptionFilter.sendError` now builds its envelope as `const payload: ApiError = …`, so the error-response shape is compile-time-checked against the documented contract — the error twin of the already-used `ApiSuccess<T>` (`import type` keeps it a type-only dependency). Intentionally **left** in place: the wired-but-unused `pushToTier`/`pushToOrg` gateway methods and `RequireTier`/`RequireRole` decorators are pre-billing/post-MVP scaffolding (CLAUDE.md: "tier gates are built … structurally complete but never triggered"), and the `HealthController`/`TimescaleService` raw-SQL calls are sanctioned infra exceptions to Rule #2 (the DB section explicitly blesses Timescale boot SQL); none are defects.

Verification: `grep -rn` for each removed symbol across `apps`/`packages` (excl. `node_modules`/`dist`) returns only its former definition; `tsc --noEmit` clean for **both** the api and shared workspaces; full api `test:unit` suite green (no suite referenced the removed symbols). Code-only — no migration, no API-contract change. Local-only follow-up (gitignored per CLAUDE.md): none.

---

## Post-MVP Pivot — F1a: Organization Tenancy (Phases A–D, 2026-06-13)

First feature of the multi-tenant pivot (full design corpus in `docs/superpowers/`, gitignored; dependency-correct execution order in `BUILD-ORDER.md`; driven on branch `feat/f1a-org-tenancy`). F1a turns NodeScope from per-user into org-tenant: organizations are real, all data is org-owned, mutations are audited, and live sync is org-wide.

- **Phase A — foundation.** New `Organization`/`OrganizationDomain`/`OrganizationMember` models + `User.isSuperAdmin` (`input:false`, server-only) + upgraded `ChangeLog` audit columns. `OrganizationsRepository`/`Service` (provision, domains, owner-designation, optimistic-concurrency update). Guards/decorators: `OrgContextGuard`, `OrgRoleGuard`, `SuperAdminGuard`, `@OrgId`, `@OrgRoles`, `@RequireSuperAdmin`. Super-admin provisioning (`/v1/admin/organizations`) + self-service (`/v1/organizations/me`). Also declared the raw-SQL PostGIS `Device.location` as `Unsupported("geometry(Point, 4326)")` so `prisma migrate` stops emitting spurious column drops (only a 1-line GIST-index strip remains per migration).
- **Phase B — org-scope every entity.** `Device`/`Network`/`DeviceConnection`/`FiberRun`/`Circuit`/`DeviceMetric` moved from `userId`-owned to `organizationId`-owned (`userId` kept as nullable creator), incl. the map raw SQL, AI network-context, realtime gateway (`socket.data.orgId` + `org:{orgId}` rooms), onboarding, clients, users. Org-configurable device naming policy (`ORG_005` name-taken / `ORG_006` pattern). `OrgContextGuard` registered globally. Greenfield migration + org-aware seed. Resolved an Orgs↔Conflict↔Realtime circular dependency via `forwardRef`.
- **Phase C — audit trail.** Request-scoped `AsyncLocalStorage` context (requestId + actor IP/UA from a global middleware, `userId` from `AuthGuard`); `AuditService`/`ChangeLogRepository`; every entity create/update/delete writes an append-only `ChangeLog` row with the org + actor taken from the session (never the request body). Fixed the init `changelog_entity_type_check` (it had omitted `Network`).
- **Phase D — org-wide live sync.** Entity mutation events broadcast to the `org:{organizationId}` room (teammates see each other's changes); AI conversations stay per-user; cross-org isolation test added.

**Known, by-design gaps (not bugs):** super-admin provisioning is the only way an org comes to exist in F1a (per spec), so fresh signups have no org and the MVP onboarding/browser-collector paths are inert until provisioning — F1b adds invitations/request-to-join. `ORG_003` is overloaded (INSUFFICIENT_ORG_ROLE vs USER_ALREADY_IN_ORG); worth a dedicated code later.

**Verification:** 34 commits; full api suite green — **unit 358 / integration 99 / e2e 118**, `tsc --noEmit` clean. Next on the roadmap: **F1b**, then the **F2→F3** permissions track and the **Spec 1→2→3** spatial/3D track (see `docs/superpowers/BUILD-ORDER.md`).

---

## Post-MVP Pivot — F2: Sites & Node Grouping (Phases A–B, 2026-06-13/14)

Third pivot feature (branch `feat/f2-sites-node-grouping`, on top of F1a and the **F1b** membership-growth feature — invitations, request-to-join, member role/removal, `ORG_009`–`ORG_015` — which landed via PR #25). F2 makes the network model *located*: it introduces an org-owned **`Property` site tree** and turns **`Device` into the Network×Site junction**, with **containment** enforced by network charters. Spec: `docs/superpowers/specs/2026-06-08-f2-sites-node-grouping-design.md`.

- **Phase A — the property tree.** New self-referencing `Property` model + `PropertyType` (`SITE → BUILDING → FLOOR → AREA`), org-scoped with optimistic `version`. `PropertiesRepository` (recursive-CTE `getSubtreeIds`/`getAncestorIds`/`isAtOrUnder`), `PropertiesService` (CRUD, type-nesting rules, cycle-safe reparent, delete-block on non-empty), `PropertiesController` (`/v1/properties`), `PropertyDto` + `v1:property:{created,updated,deleted,moved}` events. Codes `PROP_001` PROPERTY_NOT_FOUND / `PROP_002` INVALID_PARENT_TYPE / `PROP_003` PROPERTY_NAME_TAKEN / `PROP_004` PROPERTY_NOT_EMPTY / `PROP_005` PROPERTY_CYCLE.
- **Phase B — node attachment & charters.** The migration retires `BROWSER_CLIENT` and `Device.browserDeviceId`; `Device.networkId` becomes `NOT NULL` and gains a required `propertyId` + optional `roleCode` (both relations `onDelete: Restrict`); the F1a-reserved `Network.propertyId` is **dropped** (a single FK can't express a many-to-many footprint). New **`NetworkProperty`** charter (which sites a network serves) with repository/service/controller (`/v1/networks/:id/properties`, OWNER/ADMIN mutations) and `v1:network:charter:{added,removed}` events. **`ContainmentService`** owns three invariants: device create/move must sit at/under one of the network's chartered sites (`PROP_007`), charter removal can't orphan a placed device (`PROP_008`), and property reparent can't break containment for any device in the moved subtree. `PropertiesService.deleteProperty` delete-block extended to the whole subtree (children, devices, *and* charters → `PROP_004`). Codes `PROP_006` CHARTER_EXISTS / `PROP_007` DEVICE_NOT_IN_CHARTERED_SITE / `PROP_008` CHARTER_IN_USE.
...
## Post-MVP Pivot — F3 Phase A: Permission Models & Authorization Seam (2026-06-14)

F3 introduces the **assignment axis** (which site subtrees a member is scoped to) on top of F1a's existing role axis. Phase A is **purely additive** — it adds the data model and the `PermissionsService` seam but changes no existing endpoint's behavior (scoped enforcement is Phase B, management endpoints Phase C, realtime Phase D).

- **A1 — schema.** Four org-scoped models added to `schema.prisma`: `Team` (reusable assignment unit; `creatorMemberId` + `version`; case-insensitive org-unique `name` via the raw-SQL `team_org_name_lower_uniq` functional index), `TeamMember` (member↔team M2M), `TeamProperty` (team→assigned-subtree-root, `onDelete: Restrict` on `Property`), `MemberProperty` (direct per-member grant). Back-relations added to `Organization`/`OrganizationMember`/`Property`. Migration `f3_permission_models` extends the `ChangeLog` entity-type CHECK to accept `Team`/`TeamMember`/`TeamProperty`/`MemberProperty` — **retaining all existing F1a/F2 values incl. `Network`** (an early plan draft had dropped it; corrected during build). Applied to dev + test DBs; client regenerated.
- **A2 — shared DTOs + WS constants.** `TeamDto`/`TeamMemberDto`/`TeamPropertyDto`/`MemberPropertyDto`/`AccessSummaryDto` added to `packages/shared` (`AccessSummaryDto.role` typed as the re-exported `OrgRole`); nine `v1:team:*` / `v1:member:property:*` lifecycle event constants added to `WS_EVENTS` (emitted in Phase C/D). Error codes `TEAM_001`–`TEAM_002`, `PERM_001`–`PERM_005` reserved (the API Design doc is local-only/gitignored, so registration is deferred to that copy).
- **A3 — `PermissionsRepository`.** `findMember`, `createTeam`, `addTeamMember`/`addTeamProperty`/`addMemberProperty`, and `effectiveRootPropertyIds(orgId, memberId)` — the union (deduped) of a member's team-assigned roots (via team membership) and direct `MemberProperty` grants. Integration-tested against the real test DB (incl. the P2002 from the functional unique index).
- **A4/A5 — `PermissionsService` core.** `effectiveRoots`, `scopePropertyIds` (each root expanded via F2's `subtreePropertyIds`, deduped), `inScope`, the spec §5 write decision `assertCanConfigure(member, governingSiteId)` (OWNER → allow; MEMBER → `ORG_003`; ADMIN → in-scope-or-`PERM_001`), and `scopeFilter(member)` (`{ propertyIdIn } | null`; `null` = unscoped OWNER). Unit-tested with mocked repo + PropertiesService.
- **A6 — `GET /v1/access/me`.** Returns `AccessSummaryDto` (`role`, `assignedRootPropertyIds`, `unscoped`) for the caller's active org. `PermissionsModule` (imports `PrismaModule` + `PropertiesModule`; exports the service + repo) wired into `AppModule`. e2e: OWNER unscoped, MEMBER lists team-assigned roots, unauth → 401.

**Integration notes for later phases:** the global `OrgContextGuard` currently populates `request.orgMember` as `{ organizationId, role }` only (no member `id`); Phase B's `@OrgMember()` path needs the member `id`, so **Phase B will extend the guard to include it** (additive — existing F1a/F2 consumers read only `organizationId`/`role`). Phase B will also update CLAUDE.md's authorization-posture note when scoped enforcement replaces F2's MEMBER-read-only gate. F2's `PropertiesService.subtreePropertyIds(orgId, id)` is the scope-expansion seam; `NodeScopeException(code, message, statusCode)` is the throw shape.

**Verification:** full api suite green — **unit 421 / integration 124 / e2e 180**, `tsc --noEmit` clean. Phase A added 5 unit + 3 integration + 3 e2e tests; all pre-existing suites pass unchanged (additive phase).

---

## Post-MVP Pivot — F3 Phase B: Site-Scoped Enforcement on Reads & Writes (2026-06-14)

F3 Phase B **replaces** F2's interim coarse posture (MEMBER read-only / OWNER+ADMIN mutate org-wide) with the spec §5/§6 scoped decision, threaded through every entity service. The repository layer stays the single chokepoint. **Reads** apply `scopeFilter` (out-of-scope rows are invisible → 404 on direct get); **writes** find the entity org-wide then run the scoped check, so an out-of-scope ADMIN write returns **`PERM_001` (403)** (not a 404) — reads are invisible, writes are forbidden (spec §11).

- **B0 — foundation.** `OrgMemberContext` gained `id` (the `OrganizationMember.id`); `OrgContextGuard` now populates `{ id, organizationId, role }` (additive — existing consumers read only `organizationId`/`role`). New `@OrgMember()` param decorator. `PermissionsService` methods retyped to `OrgMemberContext`.
- **Decoupling (no circular dep).** Injecting `PermissionsService` into `PropertiesService` would have created a `PermissionsModule ↔ PropertiesModule` cycle (Permissions had layered on `PropertiesService.subtreePropertyIds`). Resolved by moving the subtree recursive-CTE into `PermissionsRepository.subtreePropertyIds`; `PermissionsModule` now imports only `PrismaModule`, and entity modules import `PermissionsModule` one-way (no `forwardRef`). Deviates from spec §10.2's "layered on PropertiesService" wording but preserves the functional contract and is cleaner.
- **B1 — Devices** (template). Reads scoped on `propertyId`; writes `assertCanConfigure(member, device.propertyId)` (move also asserts the target site).
- **B2 — Properties.** Governing site = the node **itself** (reads scoped on `id`); top-level `SITE` create (`parentId=null`) is OWNER-only via a `'__root__'` sentinel; reparent asserts the destination too. (Also fixed an `id`-column scope-spread collision in `findByIdAndOrgId`.)
- **B3a — Networks (shared-network rule).** Visible if **any** charter OR placed device is in scope; network-level ops (`update`/`delete`/`set-home-ip`) need **every** chartered site in scope → else **`PERM_004`** (`assertNetworkFullCoverage`). One network per org (`MAX_NETWORKS_PER_ORG`). Onboarding's `persistNetworkFields` now creates the first network via `networksRepository.create` directly (it already used the repo for updates; `if (!existing)` preserves the 1-per-org cap; the `@OrgRoles('OWNER','ADMIN')` controller gate remains) — a deliberate simplification that drops the audit/event on first-run network creation (subsequent edits via the API are still audited).
- **B3b — Charters** (`NetworkProperty`). add/remove require full coverage of the network's existing charters (`PERM_004`); add also needs the new site in scope (`PERM_001`); list shows only in-scope charters.
- **B4a — Connections + FiberRuns (inter-site-link).** Visible if **either** endpoint device's site is in scope; create/edit/delete require **both** endpoint sites in scope → else `PERM_001` (`assertBothEndpoints`).
- **B4b — Circuits.** Single optional `deviceId`: governing site = the device's site, or the `'__nosite__'` sentinel when device-less → **device-less circuits are OWNER-only** (visibility + writes), mirroring the top-level-SITE rule. Cursor list scoped on the linked device's site.
- **B5 — coarse gate retired.** The blanket `@OrgRoles('OWNER','ADMIN')` mutation gate is gone from all seven entity controllers (devices/properties/networks/connections/fiber-runs/circuits + the charter controller). The global `OrgRoleGuard` stays registered for the org-level role gates that remain (`members`, `invitations`, `join-requests`, `organizations` PATCH /me, `onboarding` turn) — those are role-, not site-, scoped.

**Verification:** full api suite green — **unit 440 / integration 124 / e2e 229**, `tsc --noEmit` clean. Phase B added ~49 e2e + ~19 unit scoped-enforcement tests across the seven entity modules; no pre-existing behavior assertion was weakened (writes-find-org-wide means a MEMBER write to an existing entity returns `ORG_003`, to a non-existent one returns 404).

---

## Post-MVP Pivot — F3 Phase C: Teams, Delegation & Admin-Authoring (2026-06-14)

F3 Phase C builds the **management surface** for the assignment axis: team CRUD, team membership, team & direct member site-assignment, the in-scope-slice access view, and the invite-authorization overlay — all enforcing the spec §7 escalation-safe delegation rules with `ChangeLog` audit. New work lives in the `permissions` module (`TeamsController`, `MemberAssignmentsController`, use-cases on `PermissionsService`, writes on `PermissionsRepository`).

- **C1 — delegation helpers** (pure, unit-tested). `assertWithinGrantorScope` (sites ⊆ actor's scope → else `PERM_002`), `assertCanManageMember` (OWNER any; ADMIN→MEMBER only → else `PERM_003`), `assertCanManageTeamStructure` (rename/delete/assign-sites: ADMIN iff **creator** AND sites ⊆ scope), `assertCanManageTeamMembership` (add/remove members: ADMIN iff the team's sites are all in scope — creator or not).
- **C2 — Team CRUD.** `GET/POST/PATCH/DELETE /v1/teams` (`creatorMemberId` = the creating admin; case-insensitive org-unique name → `TEAM_002`; `version` optimistic concurrency → `SYNC_001`; `TEAM_001` not-found). MEMBER create → `ORG_003`; non-creator ADMIN rename/delete → `PERM_003`. `ChangeLog` audit on create/rename/delete (`AuditService` is `@Global`).
- **C3/C4 — team membership + site-assignment** (sub-resources on `TeamsController`). `POST/DELETE /v1/teams/:id/members` (membership gate: team ⊆ actor scope; target must be a MEMBER → `PERM_003`); `POST/DELETE /v1/teams/:id/properties` (structure gate: creator-only for ADMIN; new site ⊆ scope → `PERM_002`). Idempotent adds; audit on each mutation.
- **C5 — direct member grants + in-scope-slice view.** `POST/DELETE /v1/members/:memberId/properties` (`MemberProperty`; ADMIN→MEMBER + site ⊆ scope). `GET /v1/members/:memberId/access` returns the target's assigned roots **as seen by the actor** — an OWNER sees all; an ADMIN sees only the slice within their own scope (out-of-scope grants are hidden).
- **C6 — invite-authorization overlay** (on F1b). `InvitationsService.create` now takes the actor's role: an ADMIN may invite at role `MEMBER` only (`ADMIN`/`OWNER` invite → `PERM_003`); OWNER any role; MEMBER → `ORG_003` (defensive; the controller's `@OrgRoles` already blocks it).
- **C8 — `PERM_005` property-delete guard.** `PropertiesService.deleteProperty` now blocks deleting a site that is **assigned** to any team or member (`countAssignmentsUnder` over the subtree → `PERM_005 PROPERTY_ASSIGNED` 409), extending F2's `PROP_004` block and surfacing the Phase-A `onDelete: Restrict` as a clean coded error.
- **C7 (folded in).** ChangeLog audit was written inline in each C2–C5 use-case (the entity-type CHECK was extended in Phase A). The `v1:team:*` / `v1:member:property:*` WS events are **deferred to Phase D**, where they're emitted with the scope-filtered fan-out from the start (avoiding an org-wide-then-rescope double pass).

**Verification:** full api suite green — **unit 450 / integration 124 / e2e 273**, `tsc --noEmit` clean. Phase C added the team/assignment management endpoints + delegation invariants (`PERM_002`/`PERM_003`/`PERM_005`, `TEAM_001`/`TEAM_002`) with ~44 new e2e + ~9 unit tests; all pre-existing suites pass unchanged.

---

## Post-MVP Pivot — F3 Phase D: Realtime Scope-Filtered Fan-Out (2026-06-14)

F3 Phase D stops broadcasting every entity event to the whole org room: a socket now receives a site-bound event only if the event's governing site is within the socket's assigned scope (OWNER receives all), and `v1:access:changed` re-syncs a socket whose teams/assignments/role change. Realtime visibility now mirrors the Phase B read filter.

- **Design choice — per-socket filter (not per-root rooms).** The spec sketched per-assigned-root rooms; this build uses an equivalent, simpler mechanism behind the same `emitScoped` seam: each socket caches its `effectiveRoots` on connect (`null` = OWNER/unscoped; `[]` = no access; `[...]` = scoped root ids), and an emit sends only to sockets whose roots intersect the **ancestor-or-self chain** of the event's governing site (so a socket scoped to root `r` receives an event at site `S` iff `r` ∈ ancestors(`S`) iff `isAtOrUnder(S, r)`). Same guarantee, no room-lifecycle churn (re-cache on access change instead of re-joining rooms). Works across instances via the existing Socket.io Redis adapter (`fetchSockets()` exposes `RemoteSocket.data`).
- **D1 — `ancestorPropertyIds`.** Upward recursive CTE on `PermissionsRepository` (mirror of `subtreePropertyIds`), org-guarded on the recursive join. Integration-tested.
- **D2 — gateway plumbing.** `IRealtimeService` gains `emitScoped(orgId, site, event, payload)`, `emitScopedMulti(orgId, sites[], event, payload)`, `notifyAccessChanged(orgId, userId)`. `RealtimeGateway` caches `effectiveRoots` per socket on connect (via `PermissionsService`/`PermissionsRepository`), filters recipients, and adds a `resync` message handler that re-caches. `ConflictResolutionService` gained delegating `emitScoped`/`emitScopedMulti` (timestamped). `WS_EVENTS.ACCESS_CHANGED = 'v1:access:changed'`. Devices routed first as the template; a `socket.io-client` e2e (`realtime-scope.e2e.ts`) **proves** a MEMBER scoped to `sB` receives **zero** events for an `sA` device mutation while the `sA` MEMBER and the OWNER each receive exactly one.
- **D3 — full network model routed.** Properties (`emitScoped` on the property's own id; delete emits before the row is removed; move emits to `[id, oldParentId]`), Networks (`emitScopedMulti` over chartered sites), Connections/FiberRuns (`emitScopedMulti` over both endpoint sites), Circuits (`emitScoped` on the device's site, or `'__nosite__'` ⇒ OWNER-only), and charters (`emitScopedMulti` over the network's charter sites incl. the affected one). Sweep confirms no site-bound event still uses the org-wide `emitEntityEvent` — that path now carries only org-level events (members/invitations/join-requests/org-settings/onboarding).
- **D4 — `v1:access:changed` + lifecycle events.** The Phase C mutations (`createTeamFor`/`renameTeamFor`/`deleteTeamFor`/`addMemberToTeam`/`removeMemberFromTeam`/`assignSiteToTeam`/`unassignSiteFromTeam`/`grantSiteToMember`/`revokeSiteFromMember`) now emit the scoped `v1:team:*` / `v1:member:property:*` lifecycle events and fire `notifyAccessChanged` for every affected user (a team-site change notifies all team members, via `teamMemberUserIds`). **Cycle resolution:** since `RealtimeModule` imports `PermissionsModule`, `PermissionsService` resolves the realtime service at runtime through **`ModuleRef.get(REALTIME_SERVICE, { strict: false })`** rather than importing `RealtimeModule` (a `forwardRef` cycle deadlocks NestJS boot in this codebase — see Phase B). An e2e (`realtime-access-changed.e2e.ts`) confirms a MEMBER's socket receives `v1:access:changed` after the OWNER grants them a site.
...
## Post-MVP Pivot — F3 Read-Scope Follow-ups Closed (2026-06-14)

Both read-scope leaks flagged in the F3 final review above are now fixed — every read across the network model is site-scoped, closing the spec §6 "every read scopes" gap. The pattern is identical on both surfaces: resolve the caller's `OrganizationMember`, call `PermissionsService.scopeFilter(member)` (OWNER → `null` = unscoped; a scoped role → `{ propertyIdIn }` = the subtree-union of its assigned roots; non-member → sees nothing), and thread the result into the entity queries.

- **Map bbox endpoints** (`d212768`). A MEMBER/ADMIN previously saw every org device (incl. `ipAddress`/`macAddress`/`notes`) on the map because the bbox queries filtered by `organizationId` only. `MapService` now injects `PermissionsService` and calls `scopeFilter(req.orgMember)`; `MapRepository` appends a `propertyId IN (...)` filter (via `Prisma.sql`) to all three bbox queries (devices/fiber-runs/circuits) — `null` skips the filter for OWNER. The devices query was also ported from `$queryRawUnsafe` to `Prisma.sql` template tags. Tests: +`map.controller.e2e`, +`map.repository.spec`, +`map.service.spec`.
- **AI assistant context** (this commit). The AI system-prompt's "Documented Network" section was the last org-wide read — a scoped member asking the assistant got the whole org's devices/links/circuits in context. `NetworkContextProvider` now resolves scope itself (`PermissionsRepository.findMember(orgId, userId)` → `scopeFilter(member)`) because `ContextBuilderService` hands it only `(organizationId, userId)`, not the request's `orgMember`. The resolved `scopeIds` (`null` = OWNER, `[]` = non-member/unassigned, `[...]` = scoped) thread into `NetworkContextRepository.getNetworkEntities`, which filters devices by `propertyId`, connections/fiber-runs by **either** endpoint device's `propertyId`, and circuits by their device's `propertyId`. `AiModule` now imports `PermissionsModule` (`PermissionsService`'s `AuditService` dep resolves via the `@Global()` `AuditModule`); `jest.unit.config.ts` testRegex broadened to also match `*.provider.spec.ts`. New unit spec `network-context.provider.spec.ts` (5 cases: OWNER→unfiltered/`null`, scoped ADMIN→`propertyIdIn`, non-member→empty-list without calling `scopeFilter`, + output assembly). `RealtimeContextProvider` needs no change — it reads only the caller's own browser-session metric, not org-wide data.

**Verification:** full api suite green — **unit 456 / integration 126 / e2e 278**, `tsc --noEmit` clean. **F3 read-scope follow-ups closed; F3 fully complete (Phases A–D + final-review fixes + both read-scope follow-ups).**

---

## Post-MVP Pivot — Spec 1 (Spatial Foundation) Phase A — Models, Storage Seam & Infra (2026-06-15)

First task of the 3D/spatial track (Spec 1 → 2 → 3 → 4, converges with the permissions track at Spec 4). Phase A lays the server-side substrate — versioned building models in object storage + model-local device 3D coords — with **no HTTP surface yet** (model API + proxied transfer = Phase B; device-position endpoint = Phase C). Branch `feat/spec1-spatial-foundation` off the merged F3 master (`9eba05d`).

- **Infra (`b0d7b07`).** `@aws-sdk/client-s3` + `@aws-sdk/lib-storage`; **MinIO** wired into `docker-compose.yml` (dev `:9000`/`:9001`) and `docker-compose.test.yml` (`minio-test` `:9100`, tmpfs); `STORAGE_*` in `.env.example`; `STORAGE_*` test defaults in `jest.e2e.setup.ts`.
- **Schema (`6d7f935`).** `BuildingModel` (one per BUILDING, points at active version) + immutable `BuildingModelVersion`, two named relations (`ActiveVersion`/`ModelVersions`); `Device.x/y/z` nullable (model-local meters, 2D fields untouched). Migration stripped the spurious `DROP INDEX device_location_idx` (GIST index Prisma can't model) and extended the `ChangeLog` entityType CHECK to a **superset** of the current F3 list + the two new types (the plan's pre-F3 list would have dropped `Network`/`Team*`). **Plan deviations (flagged):** `propertyId`/`activeVersionId` made `@unique` so `Property.buildingModel` and `BuildingModelVersion.activeForModel` are valid singular 1:1 back-relations (a composite `@@unique` doesn't satisfy Prisma's 1:1 rule; the plan also omitted the `ActiveVersion` opposite field) — functionally identical. Applied to dev (`:5432`) + test (`:5433`); the four service-spec mock `Device` literals gained `x/y/z`.
- **Shared (`d0a9651`).** `BuildingModelDto`, `BuildingModelVersionDto` (no `storageKey` — internal), `DevicePositionDto`; `DeviceDto.x/y/z`; `v1:buildingModel:{versionUploaded,activated,deleted}`; both `DeviceDto` assemblers (`devices.service`, `map.service`) map x/y/z. `MODEL_*`/`SPATIAL_*` codes are enumerated authoritatively in the Spec 1 design §10.3 and thrown starting Phase B — `API_Design.md` registration deferred (that doc isn't materialized in this worktree).
- **StorageService (`0816db5`).** The sole S3/MinIO bucket accessor (`buildVersionKey`, `ensureBucket`, `putObjectStream`, `getObjectStream`, `deleteObject`, `objectExists`); `S3_CLIENT` injected via token (unit-mocked). 4 unit tests.
...
## Post-MVP Pivot — Spec 1 (Spatial Foundation) Phase C — Device 3D Coordinates (2026-06-15)

The device-position endpoint + building-resolution rules + clear-on-building-change hook, on `feat/spec1-spatial-foundation`. All org-scoped, OWNER/ADMIN-gated.

- **`SpatialRepository` (Task 1).** Two methods: `findDevice(orgId, deviceId)` (org-scoped device lookup); `resolveGoverningBuildingId(orgId, propertyId)` — a recursive CTE walking `Property` parent links upward from the device's `propertyId`, returning the nearest ancestor-or-self of type `BUILDING`, or `null`. `setPosition(orgId, deviceId, x, y, z)` — `updateMany` + re-read (same non-atomic pattern as `DevicesRepository.updateWithVersion`; acceptable for non-critical coordinate writes). 1 integration suite (real test DB).
- **`SpatialService` (Task 2).** Enforces the two rules before writing: `SPATIAL_002 INCOMPLETE_POSITION` (422) when only some of `x/y/z` are set (all-or-nothing — clear is always allowed); `SPATIAL_001 DEVICE_NOT_IN_MODELED_BUILDING` (422) when coords are being set but the device's governing BUILDING has no active `BuildingModel`. Uses `BuildingModelsRepository.findByProperty` (imported directly, no cross-module injection risk — `BuildingModelsModule` is already wired). Returns `DeviceDto` via the shared `toDeviceDto` mapper. 1 unit suite.
- **`SpatialController` + `toDeviceDto` mapper extraction (Task 3).** `PATCH /v1/devices/:id/position` (`@OrgRoles('OWNER','ADMIN')`, `@OrgMember()`, `ParseUUIDPipe`). Body: `DevicePositionInputDto` (`x?`, `y?`, `z?` — all optional floats). `toDeviceDto` extracted from `DevicesService` into `apps/api/src/devices/device.mapper.ts` so `SpatialService` can map without a circular import — both `devices.service.ts` and `map.service.ts` updated to import from the new location. `SpatialModule` → `AppModule`. 1 e2e suite (real test DB + real MinIO :9100).
- **Clear-on-building-change hook (Task 4).** In `DevicesService.updateDevice`: when a `PATCH /devices/:id` changes `propertyId`, the governing BUILDING of the old and new property are resolved in parallel via `SpatialRepository.resolveGoverningBuildingId`; if they differ, `x/y/z` are set to `null` in the update payload before `updateWithVersion` writes. `SpatialRepository` is now a constructor dep of `DevicesService`. The devices.service unit spec was updated to mock `SpatialRepository` (clear-on-move branch + no-op when same building); the `devices.repository.spec.ts` audit-integration block gained `SpatialRepository` in its provider list (the missing mock caused 1 failing test at Phase C gate — fixed in this phase).
- **`DeviceDto.x/y/z`** (already in shared from Phase A) — `toDeviceDto` in `device.mapper.ts` maps all three fields; both the device-list and map-devices responses carry them as of Phase A.

**`SPATIAL_*` error codes** (`SPATIAL_001 DEVICE_NOT_IN_MODELED_BUILDING`, `SPATIAL_002 INCOMPLETE_POSITION`) are enumerated in the Spec 1 design §10.3 and in `docs/superpowers/API-REFERENCE.md` (gitignored local copy, already up to date; `PATCH /devices/:id/position` is in the surface table at line 41 of that file). `API_Design.md` registration deferred (not materialized in this worktree — update belongs on `share/local-docs`).

**Seed addition:** `prisma/seed.ts` gains a standalone `seedBuildingModel(organizationId)` function (called from `main()` after `seedNetworkData`) that: looks up the `Main Building` `BUILDING` property by name; guards idempotently on `BuildingModel` existence; constructs the S3 client from the same `STORAGE_*` env vars read inline (defaulting to the dev MinIO at `http://localhost:9000`); calls `CreateBucketCommand` (idempotent); uploads a minimal valid IFC (`ISO-10303-21;\n…`) under `org/{orgId}/building/{propertyId}/{uuid}.ifc`; creates the `BuildingModel` + `BuildingModelVersion` rows (versionNumber 1, sha256 + byteLength computed inline, `uploadedByMemberId: null`); activates the version; and sets `x=5.0 y=3.2 z=1.5` on the `Core Switch` device (which sits on `Ground Floor` → `Main Building`). Safe to re-run: both runs log their skip reason and return without error.

**Phase gate:** full api suite green — **unit 479 / integration 132 / e2e 292**, `tsc --noEmit` clean. The integration count grew by 3 (spatial repository) and the e2e count by 7 (spatial e2e suite). The failing integration test at gate (`devices.repository.spec.ts` audit block missing `SpatialRepository` mock) was introduced by Task 4 and fixed in this phase.

---

## Post-MVP Pivot — Spec 2 (Desktop Shell) Phase A — PKCE Auth Bridge (2026-06-15)

System-browser PKCE bridge between the future Electron desktop app and the existing Better Auth session layer. Branch `feat/spec2-desktop-shell`.

- **`@nodescope/client` package (`16dbba5`, `7980a26`, `9ffd828`).** New `packages/client` workspace — the Electron app's typed SDK. REST client: `createRestClient(baseUrl, getToken)` — Axios instance with `Authorization: Bearer` injected per-request, returns typed `ApiResponse<T>`, throws `ApiError` on non-2xx. Realtime client: `createRealtimeClient(baseUrl, getToken)` — socket.io-client 4.x with auth handshake token; `on`/`off`/`emit`/`connect`/`disconnect` wrappers. Re-exports `@nodescope/shared` types. Vitest suite: 3 tests (rest: 200 happy-path + 401 error-throws; realtime: auth option injected). `postinstall` script updated to build `@nodescope/client` after `@nodescope/shared` (client depends on shared).
- **Better Auth bearer plugin (`abc2ab1`).** `bearer()` added to `apps/api/src/auth/better-auth.config.ts` plugins. Session tokens now work as `Authorization: Bearer <token>` on every endpoint — no per-route change. The existing global `AuthGuard` calls `auth.api.getSession` which resolves bearer tokens transparently once the plugin is enabled.
- **`DesktopAuthService` (`e8b129d`, `63a5f78`).** PKCE S256 one-time-code store in Redis (`desktop-auth:code:{code}`, 120 s TTL). `issueCode({ sessionToken, challenge })` — `randomBytes(32).base64url` code, stores `{ sessionToken, challenge }` JSON. `exchange(code, verifier)` — `get` → `del` (burn before verify — prevents brute-force reuse) → PKCE S256 verify via `timingSafeEqual`; returns the session token (the bearer credential). Error codes: `DAUTH_002 CODE_INVALID_OR_EXPIRED` (400), `DAUTH_003 PKCE_VERIFICATION_FAILED` (400). 1 unit suite (4 tests + the burn-before-verify invariant).
- **`DesktopAuthController` + `DesktopAuthModule` (`91387cb`).** `GET /v1/desktop-auth/authorize` (`@Public`) — validates `redirect_uri` (allow-list: `nodescope://auth/callback` only; else `DAUTH_001 INVALID_REDIRECT_URI` 400); if no session, redirects browser to `FRONTEND_URL/login?returnTo=<full-API-authorize-URL>`; if session, issues code and redirects to `nodescope://auth/callback?code=…&state=…`. `POST /v1/desktop-auth/token` (`@Public`) — exchanges code + verifier for bearer token (201). `POST /v1/desktop-auth/revoke` (AuthGuard-protected) — calls `auth.api.signOut` (204). Module: `imports: [RedisModule]`, `controllers: [DesktopAuthController]`, `providers: [DesktopAuthService]`. `DesktopAuthModule` added to `AppModule.imports`. 1 e2e suite (7 cases: full PKCE round-trip + bearer-authenticated call, no-session redirect asserts full API URL in `returnTo`, `DAUTH_001` bad-redirect, `DAUTH_003`+`DAUTH_002` wrong-then-reused-verifier, revoke-401-without-token, revoke-204-with-token, DTO validation).
- **Web login `returnTo` (`3019566`).** `apps/web/lib/safe-desktop-return.ts` — `safeDesktopReturnTo(raw, apiOrigin)`: accepts only absolute URLs whose origin matches the API origin and whose path starts with `/api/v1/desktop-auth/authorize`; all other inputs (cross-origin, javascript:, relative path, wrong path) return `null`. `login.tsx` reads `?returnTo` from `window.location.search` post-sign-in, validates via `safeDesktopReturnTo`, and does `window.location.href = returnTo` (full-page navigation to the API, preserving the cookie) instead of `router.replace`. Controller updated to embed the full `BETTER_AUTH_URL`-prefixed URL so the web app (different origin in prod) redirects to the correct host. Web helper test suite: 8 cases (7 rejection + 1 acceptance). **`DAUTH_*` codes** are enumerated authoritatively in the Spec 2 design; `API_Design.md` registration deferred (not materialized in this worktree — update belongs on `share/local-docs`).

**Phase gate:** full api suite green — **unit 481 / integration 132 / e2e 300**, `tsc --noEmit` clean. Client vitest: **3 passed**. Web `safeDesktopReturnTo` spec: **8 passed**. Web `tsc --noEmit`: 4 pre-existing `BROWSER_CLIENT`/`browserDeviceId` errors (F2 pivot, out of scope) — no new errors. `npm run lint`: 0 errors (9 pre-existing `react-hooks/exhaustive-deps` warnings, unchanged). **Spec 2 Phase A complete.**

---

## Post-MVP Pivot — Spec 2 (Desktop Shell) Phase B — Electron Scaffold & Auth (2026-06-15)

The `apps/desktop` Electron app (electron-vite: main + preload + React-DOM renderer) + the full desktop auth flow in the main process, consuming Phase A's `/v1/desktop-auth/*`. Branch `feat/spec2-phase-b-electron` (stacked on Phase A). Electron 32 installs + runs headless here (`electron-vite build`, Vitest units, and Playwright-Electron all run without a display; only the live system-browser OAuth round-trip needs manual verification).

- **electron-vite scaffold (`093680f`).** `apps/desktop` workspace: `electron.vite.config.ts` (main/preload/renderer targets, `@vitejs/plugin-react`), project-reference tsconfigs (`tsconfig.node.json` for main+preload, `tsconfig.web.json` for the DOM renderer; `lint` = `tsc -b --noEmit`). Minimal `BrowserWindow` (`contextIsolation`/`sandbox`/no-`nodeIntegration`), preload stub, React 19 renderer stub. Verified via `electron-vite build` (headless) — no visual window check.
- **Auth core (`fa279cb`, `ebadcff`, `a9f4b7c`, `9dc1188`).** All in `src/main/auth/`, all Vitest-unit-tested headless. `pkce.ts` `createPkce()` → S256 (`base64url(sha256(randomBytes(32)))`). `token-vault.ts` `TokenVault(file)` — `save` (guards `safeStorage.isEncryptionAvailable()` → throws `VAULT_ENCRYPTION_UNAVAILABLE`; else `encryptString` → `fs.writeFile`), `load` (decrypt, null on miss/corrupt), `clear`. `auth-flow.ts` `AuthFlow({apiUrl,openExternal,fetchFn,vault,onChange})` — **fully injected deps** for headless tests; `login()` (PKCE + random 16-byte `state` → `openExternal` the authorize URL), `handleCallback(rawUrl)` (guard `nodescope://auth/callback` host+path, **state-match CSRF check**, POST code+verifier to `/v1/desktop-auth/token`, `vault.save`, `onChange(true)`; throws `AUTH_STATE_MISMATCH`/`AUTH_EXCHANGE_FAILED`; ignores non-callback deep links), `getToken`/`logout`. 9 unit tests (incl. CSRF rejection, deep-link ignore, null-pending, exchange-failure-saves-nothing). Security-reviewed.
- **Main wiring + preload bridge (`fc01755`).** `main/index.ts`: single-instance lock (quit if second instance), `setAsDefaultProtocolClient('nodescope')`, `second-instance` (Win/Linux argv) + `open-url` (macOS) → `flow.handleCallback(url).catch(()=>{})` (malformed deep link can't crash), IPC handlers `auth:{login,logout,getToken}` + `app:getConfig`, hardened `BrowserWindow`. `main/config.ts` `apiUrl` from `VITE_API_URL`. `preload/index.ts`: `contextBridge.exposeInMainWorld('nodescope', {auth, app})` — exposes ONLY the 4 fixed-channel auth methods + `getConfig` (no `ipcRenderer`/`require`/`process`/generic-invoke leak); `onAuthChanged` returns an unsubscribe. `preload/api.d.ts` types `window.nodescope`.
- **Renderer + e2e (`93baefd`).** `renderer/App.tsx` — auth-aware stub (Sign-in button → `window.nodescope.auth.login()`; "Signed in" when a token exists; subscribes to `onAuthChanged`). jsdom Vitest test (2 cases) via `@vitest-environment jsdom` docblock. `e2e/login.e2e.ts` (Playwright-Electron) launches the built app and asserts the signed-out shell — exercises the real main→preload→IPC→renderer round-trip. The full browser PKCE redirect is out of e2e scope (AuthFlow units cover the exchange logic). `react-router-dom`/`zustand` are forward-declared deps for the Phase C shell (unused here).

**Phase gate:** `electron-vite build` succeeds (main 4.14 kB / preload 0.61 kB / renderer 529 kB); desktop Vitest **11 passed** (9 node main-process + 2 jsdom renderer); desktop `tsc -b --noEmit` clean; Playwright-Electron e2e **1 passed** (headless); **root `npm run lint` 0 errors**. Needs manual (display) verification: the live Sign-in → system-browser → `nodescope://` callback round-trip + OS protocol registration. **Spec 2 Phase B complete** (Phase C = the sidebar/viewport shell + `@nodescope/client` wiring + electron-builder packaging).

## Post-MVP Pivot — Spec 2 (Desktop Shell) Phase C — Shell, Connectivity & Packaging (2026-06-17)

The sidebar + viewport shell, Zustand state, `@nodescope/client` wiring, a self-host Settings override, and Windows packaging — completing the Electron desktop shell. Built on Phase B's branch (`feat/spec2-phase-b-electron` → `feat/spec2-phase-c`).

- **Zustand stores (`df5e5ae`).** `renderer/stores/`: `useAuthStore` (authed + `OrganizationDto`), `useSitesStore` (`PropertyDto[]` + `selectedBuildingId`), `useViewportStore` (`activeBuildingPropertyId`) — **the reserved Spec 3 boundary** (Spec 3 adds camera/model/selection and owns the render loop reading/writing it outside React). zustand v5 curried form. 3 vitest units.
- **Client bootstrap (`fa8fe65`).** `data/clients.ts` `buildClients()` builds the REST + realtime clients from the preload bridge (`app.getConfig()` base URL + `auth.getToken`). `data/use-bootstrap.ts` `bootstrap()` loads `getOrganization()` + `listProperties()` into the stores, connects realtime, and re-fetches the tree on `PROPERTY_{CREATED,UPDATED,DELETED,MOVED}` (`WS_EVENTS` constants); `useBootstrap()` runs it whenever auth flips true. 1 vitest unit.
- **Shell + router (`df8920f`).** `shell/Sidebar` (F2 site/building tree; selecting a building → `selectBuilding` + viewport `setActive`), `ViewportHost` (the Spec 3 placeholder: "Select a building" / "3D viewport for … (Spec 3)"), `TopBar` (org name + Sign out → `auth.logout()`), `Shell` (composes them + `useBootstrap()`). `screens/Login` (Sign in) + `Settings`. `App.tsx` → **HashRouter** with an authed guard: `/login` (unauthed) · `/` (Shell) · `/settings`; subscribes `onAuthChanged` → `authStore`. **Supersedes Phase B's direct Sign-in/Signed-in `App`** (its jsdom test was rewritten for routing). 4 shell RTL units + 2 App routing units (`useBootstrap` mocked so the authed route does no real I/O).
- **Build fix (`9062aa7`).** The renderer is the first consumer of `@nodescope/client`; these symlinked workspace packages ship as CommonJS (resolved outside `node_modules`), so the production rollup build couldn't extract their named exports. `electron.vite.config.ts`: `build.commonjsOptions.include = [/packages//, /node_modules/]` + `optimizeDeps.include` for the dev server. (Vitest was unaffected — esbuild handles CJS interop; Phase B's renderer never imported the client.)
- **Settings override (`cf4f666`).** `config.ts` `getApiUrl()` prefers a persisted `userData/settings.json` override (else `VITE_API_URL`, else dev default); `setApiUrl()` writes it. Main resolves `apiUrl` once at startup (restart-to-apply) + IPC `app:setApiUrl`; preload + `api.d.ts` add `app.setApiUrl(url)`; `screens/Settings.tsx` is the API-URL form. 1 RTL unit.
- **Windows packaging (`7c0bf4e`).** `electron-builder.yml` (NSIS x64, assisted installer, choose-dir, appId `io.nodescope.desktop`); `package:win` script; `.github/workflows/desktop-build.yml` (windows-latest, `workflow_dispatch`/`v*` tags → build + upload the installer artifact, kept off per-push CI as it's heavy). Config validated; the `.exe` requires a Windows runner.
- **Docs (this commit).** `apps/desktop/README.md` (develop / auth / test / build+package / the `<ViewportHost>`+`viewportStore` Spec 3 seam).

**Phase gate:** `electron-vite build` PASS (renderer bundle ~717 kB — now bundles `@nodescope/client` + socket.io + react-router); desktop **vitest 20 passed** (8 files: Phase B's pkce 1 / token-vault 2 / auth-flow 6 retained + Phase C stores 3 / bootstrap 1 / shell 4 / App 2 / Settings 1); `tsc -b --noEmit` clean; **Playwright-Electron e2e 1 passed** (launches to the signed-out shell). Full gate — incl. the Electron window + e2e — run in-sandbox under `xvfb` + `ELECTRON_DISABLE_SANDBOX=1` (Debian-12 root). **Spec 2 Phase C complete → the whole Spec 2 Desktop Shell (A+B+C) is done.** Next on the 3D track: **Spec 3** (the r3f viewport replacing `<ViewportHost>`).

## Post-MVP Pivot — Spec 3 (3D Viewport) Phase A — IFC Loader & ParsedModel (2026-06-17)

`IfcModelLoader` — the single wrapper over web-ifc that parses an IFC `ArrayBuffer` into the `ParsedModel` Three.js handle. Headless, node-unit-tested; no React/WebGL. Branch `feat/spec3-viewport` (stacked on `feat/spec2-phase-c`).

- **Deps + WASM + fixture (Task 1).** `web-ifc` 0.0.77 + `three` 0.184 (+ `@types/three`). `electron.vite.config.ts`: an inline plugin serves (dev) + copies (build) `web-ifc.wasm` to the renderer web root (`vite-plugin-static-copy` is incompatible with electron-vite's renderer pipeline). Fixture: a hand-written minimal single-wall IFC4 (`viewport/ifc/__tests__/fixtures/wall.ifc`, with a `Pset_WallCommon`), validated via a throwaway web-ifc spike before writing the loader.
- **`IfcModelLoader` geometry (Task 2).** `viewport/ifc/ifc-model-loader.ts` `createIfcModelLoader({wasmPath}).loadModel(bytes)` → `ParsedModel`: `LoadAllGeometry` → one merged vertex-coloured `BufferGeometry` per element (`userData {expressID, ifcType}`) parented under per-IFC-class `Group`s; **recenter** (−bbox center) + **Z-up→Y-up** (`root.rotation.x=-π/2`); `bbox`, `elementIndex`, `frame {recenter, upConversion}` (the Spec 4 coordinate contract), `dispose` (CloseModel + free buffers). `viewport/ifc/ifc-types.ts` (`ParsedModel`/`ElementProperties`/`IfcType`/`ExpressId`) is the **frozen B/C/D contract**. `tsconfig.web.json` adds `node` types for the node-env loader spec.
- **`getProperties` (Task 3).** `GetLine` Name/Tag + `properties.getPropertySets` (inlined) → `ElementProperties`; `asText` normalizes web-ifc `{value}` wrappers.
- **Docs (Task 4).** `apps/desktop/README.md` — the client-side main-thread parse posture, the bundled `.wasm`, and the `ParsedModel`/`ElementProperties` cross-phase contract.

**Phase gate:** desktop **vitest 24 passed** (9 files; +4 node loader units: geometry/categorize/index, recenter+Y-up, dispose, getProperties); `tsc -b --noEmit` clean; `electron-vite build` PASS (web-ifc.wasm 1.3 MB copied to `out/renderer`). web-ifc runs headless under node (its `node` export condition resolves to the fs-based build, so no WASM-fetch). **Spec 3 Phase A complete.** Next: **Phase B** (viewportStore extension + load lifecycle/states + pure `isMeshVisible`/`sectionToPlane`).

## Post-MVP Pivot — Spec 3 (3D Viewport) Phase B — Store, Load Lifecycle & States (2026-06-17)

The viewport state machine + load lifecycle + overlays — no WebGL yet (the `ready` slot is a placeholder Phase C swaps for the canvas).

- **Extended `viewportStore` (`2fddd6e`).** Spec 2's reserved store gains `status`/`error`/`model`/`reloadNonce`/`updateAvailable` + interaction state (`selection`/`hiddenCategories`/`isolated`/`hiddenElements`/`section`) + actions (`select`/`isolate`/`clearIsolation`/`toggleCategory` (immutable Sets)/`hideElement`/`showAll`/`setSection` (partial-merge)/`flagUpdate`/`reload`) + internal `_setStatus`/`_setModel`; `setActiveBuilding` resets per-model state; `initialViewportState()` for **merge**-resets (a `replace:true` reset would wipe the action fns — the plan's test was corrected). 5 vitest units.
- **Pure derivations (`10547f4`).** `interaction/visibility.ts` `isMeshVisible(mesh, state)` (category/element hide + isolation) + `interaction/section.ts` `sectionToPlane(SectionState)` → `THREE.Plane` (axis normal + constant; keeps the region below the cut) — both WebGL-free, testable. 6 vitest units.
- **Load lifecycle (`f233595`).** `use-viewport-loader.ts` `loadBuilding()` runs metadata→download→parse→ready against injected deps with an `isCurrent()` guard at every await (stale resolve discarded + disposed); `MODEL_001`→empty (no download); failures→error, never throws. `createModelCache()` keep-last-1 (dispose-on-evict). `useViewportLoader()` wires it to `activeBuildingPropertyId`+`reloadNonce`. `data/clients.ts` exposes `setClients`/`getClients` (bootstrap sets them). 6 vitest units.
- **Overlays + realtime (`5567ff2`).** `ui/overlays` (Idle/Loading/Empty/ErrorState/UpdateBanner); `ViewportHost` switches on status + drives the loader/realtime hooks. `use-model-realtime.ts`: `BUILDING_MODEL_{ACTIVATED,VERSION_UPLOADED}`→`flagUpdate` for the open building, `DELETED`→empty. (Spec 2's placeholder ViewportHost test moved to `overlays.spec`; the App routing test asserts the viewport region.) 4 overlay RTL + 3 realtime units.

**Phase gate:** desktop **vitest 46 passed** (15 files); `tsc -b --noEmit` clean; `electron-vite build` PASS. **Spec 3 Phase B complete.** Next: **Phase C** (the r3f `<Canvas frameloop=demand>` — ViewportCanvas/ModelView/CameraRig + OrbitControls — replacing the `ready` slot).

## Post-MVP Pivot — Spec 3 (3D Viewport) Phase C — Scene Rendering & Navigation (2026-06-17)

The building now actually renders in 3D and is navigable. Deps `@react-three/fiber` 9 + `@react-three/drei` 10 + dev `@react-three/test-renderer` 9 (React 19 / three 0.184 aligned).

- **`fitCameraToBox` (`bacf2a4`).** Pure camera framing: target the bbox centre, pull back so the bounding sphere fits the min(v,h) FOV on a 3/4 iso direction; empty-box safe. 2 node units.
- **Scene + ViewportCanvas (`6e28e22`).** `scene/Lighting` (hemisphere+directional+ambient), `ModelView` (`<primitive object={model.root}>`), `CameraRig` (fit on bbox change + demand-invalidate; OrbitControls target update), `ViewportCanvas` (`<Canvas frameloop="demand">`, `gl.localClippingEnabled` for the §8 section plane, drei `<OrbitControls makeDefault onChange=invalidate>`). Split into `Scene` (contents) + `ViewportCanvas` (Canvas wrapper) so `@react-three/test-renderer` can mount the scene headless. **Render smoke** asserts the model mesh + a HemisphereLight mount (OrbitControls stubbed — it binds to a canvas `domElement` test-renderer lacks). **r3f-v9 JSX-types fix:** the bundled main types drop the `IntrinsicElements extends ThreeElements` augmentation, so `r3f-jsx.d.ts` re-registers it on the global/`react`/`react/jsx-runtime`/`react/jsx-dev-runtime` JSX namespaces (force-included via `tsconfig.web` `files`).
...
## Post-MVP Pivot — Spec 4 (Nodes in 3D) Phase A — Foundations: Coords, Helpers, Client, Store & Selection (2026-06-17)

The headless foundation for nodes-in-3D — the contract Phases B–D consume; no markers/scene/UI yet. **Spec 4 is the convergence of the 3D + permissions tracks.** Branch `feat/spec4-nodes-in-3d`, stacked on `feat/spec3-viewport`.

- **`node-coords` (`514152f`, Task 1).** `viewport/nodes/node-coords.ts`: pure `toViewport(xyz, frame)` / `toModel(point, frame)` from one `Matrix4` `Rx(-90°)·T(-recenter)` — the same transform Spec 3 bakes into `ParsedModel.root`. Round-trip tested (`toModel(toViewport(p)) ≈ p`); native +Z → viewport +Y. The Spec 4 coordinate bridge (§5, §11). 2 node units.
- **Pure helpers (`2f957c2`, Task 2).** `node-status.ts` = the status-display seam (`NodeStatus = up|down|warning|unknown` + `STATUS_COLOR`; the Monitoring spec fills it, **v1 = `unknown`**); `can-configure.ts` = the pure F3 role gate (`canConfigure(access)` → OWNER/ADMIN true, MEMBER/null false); `filter-devices.ts` = pure `filterDevices` across text/category/network/placement/floor/status + `emptyFilter`. 6 units.
- **Server building filter (`ace4dc2`, Task 3).** `GET /v1/devices?buildingPropertyId=` → `DevicesService.listDevicesForBuilding`: resolves the F2 building **subtree** (`PropertiesService.subtreePropertyIds`) AND-ed with the caller's F3 read-scope (`scopeFilter`; `null` = OWNER → no filter), reusing `DevicesRepository.findAllByOrgId({propertyIdIn})`; an empty intersection short-circuits to `[]`. Returns a bare array (not paginated). 3 unit tests (mocked deps); the real-DB subtree query is covered by integration/e2e in the build env.
- **Client methods (`24b2da4`, Task 4).** `@nodescope/client`: `listDevicesForBuilding(buildingPropertyId)` (scoped GET) + `setDevicePosition(id, pos|null)` (Spec 1 `PATCH …/position`; `null` clears). 3 units (`vi.stubGlobal` fetch).
- **Store + tagged selection (`6bac11c`, Task 5).** `viewportStore.selection` refactored to a tagged `{kind:'element',expressID} | {kind:'device',deviceId} | null` union (`selectElement`/`selectNode`/`clearSelection`); added node state `devices`/`placingDeviceId`/`nodeFilter`/`nodeStatus`/`access` + actions (`setDevices`/`upsertDevice`/`removeDevice`/`beginPlace`/`cancelPlace`/`setNodeFilter`/`setNodeStatus`/`setAccess`); `setActiveBuilding` resets per-building node state (keeps org-level `access`). Spec 3's selection consumers (`picking`, `apply-model-state`, `ViewCommands`, `Inspector`) updated to the union — all stay green. 6 store units.

**Phase gate:** desktop **vitest 76 passed** (25 files); `tsc -b --noEmit` clean; `electron-vite build` PASS (752 modules, headless bundle); client **vitest 6 passed** (3 files); api **unit 484 passed** (48 suites; +3 `listDevicesForBuilding`) + `nest build` clean. Verified **headlessly in-sandbox** (Phase A needs no DB/display); the api **integration/e2e** (real subtree query) + **Playwright-Electron e2e** run in the build env. The `node-coords` bridge + tagged `Selection` + node-state store + client device methods are the **Phase B/C/D contract**. Next: **Phase B** (`NodeLayer` billboard markers + status ring, `useDeviceLoad` realtime, marker-priority picking).

## Post-MVP Pivot — Spec 4 (Nodes in 3D) Phase B — Node Layer, Device Load & Picking (2026-06-17)

The building's *placed* devices now render as category-coded marker sprites in the Spec 3 scene, the list loads + stays live, and markers are pick-priority over the building.

- **Device load + realtime + access (`85adec7`, Task 1).** `viewport/use-device-load.ts`: pure `loadDevicesFor(propertyId, rest, isCurrent)` (sets `store.devices`; **stale load discarded** via Spec 3's race pattern) + `applyDeviceEvent(kind, payload)` (live-patches only an ALREADY-listed device on `v1:device:updated`, removes on `:deleted`; a newly created device enters the list on the next building switch); the `useDeviceLoad()` hook loads on building change, subscribes to `v1:device:*`, and loads the caller's `AccessSummary` once into `store.access`. New client `getAccessSummary()` (`GET /v1/access/me`). Wired into `ViewportHost`. 3 units.
- **Marker raycast helpers (`e75dd45`, Task 2).** `nodes/category-color.ts` = deterministic category→palette hash; `nodes/picking-nodes.ts` = pure `pickNode(ray, markers)` → nearest marker's `deviceId` (flat leaf-sprite list, non-recursive). 3 units.
- **NodeLayer + marker-priority picking (`71f38ba`, Task 3).** `nodes/NodeLayer.tsx` (in-Canvas): one billboard `<sprite>` per *placed* device at `toViewport(xyz)`, **category-coloured** + a **status ring** (`STATUS_COLOR`, v1 `unknown`), selected node emphasised (1.6× scale); markers draw with `depthTest:false` at world positions (not under `model.root`) → an annotation layer **independent of section/isolate** (§5); each leaf sprite carries `userData.deviceId`, collected into a shared `markersRef`. `interaction/picking.ts` `PickingController` now takes `markersRef` and raycasts **markers first** (`pickNode` → `selectNode`), else the building (`pickExpressId` → `selectElement`/clear). `ViewportCanvas.Scene` mounts `<NodeLayer>` + the marker-aware `<PickingController>`. 3 render-smoke/units (`@react-three/test-renderer`).

**Phase gate:** desktop **vitest 84 passed** (29 files; +8); `tsc -b --noEmit` clean; `electron-vite build` PASS (headless bundle). Verified headlessly in-sandbox. Live orbit/marker legibility/click-to-select + GPU/visual correctness are manual per spec §13 (need display + a building with placed devices + API). `NodeLayer`'s `markersRef` + `pickNode` priority + `use-device-load` are the **Phase C/D contract**. Next: **Phase C** (select-then-click placement → surface raycast → `toModel` → `PATCH position`, optimistic + rollback; mid-air rejected).

## Post-MVP Pivot — Spec 4 (Nodes in 3D) Phase C — Placement (Place / Move / Clear) (2026-06-17)

Authorized users can now position a device by clicking the building surface — optimistic, with server-authoritative rollback.

- **Placement engine (`0962c9a`, Task 1).** `viewport/nodes/placement.ts`: pure `raycastBuildingPoint(ray, model, vis)` (nearest VISIBLE mesh hit in world space, else null) + `commitPlacement(deviceId, point, {rest, frame, notify})` (world hit → `toModel` → native xyz → **optimistic** `upsertDevice` + `cancelPlace` → `setDevicePosition` → upsert the server result; **rollback to prev + notify** on reject) + `clearPlacement(deviceId, {rest, notify})` (optimistic null → persist null → rollback on reject). Move = `commitPlacement` on an already-placed device. 4 units (mocked client, real three math). *(Note: the plan's example asserted the raw hit as the persisted value; the engine correctly persists `toModel(hit)` — the test asserts against that conversion.)*
- **Controllers + mount (`0c4722a`, Task 2).** `nodes/PlacementController.tsx` (in-Canvas): while `placingDeviceId` is set, a **capture-phase** click raycasts the surface → `commitPlacement` (no hit ⇒ no-op, mid-air rejected; **Esc** → `cancelPlace`). `interaction/picking.ts` skips selection while placing (the click is a placement). `ViewportCanvas.Scene` mounts `<PlacementController>`. 1 render-smoke (`@react-three/test-renderer`).

**Phase gate:** desktop **vitest 89 passed** (31 files; +5); `tsc -b --noEmit` clean; `electron-vite build` PASS (headless bundle). Verified headlessly in-sandbox. The full place→PATCH→persist round-trip + MEMBER-reject rollback are manual per spec §13 (need display + an OWNER/ADMIN session + a building + a device). `beginPlace`/`commitPlacement`/`clearPlacement` are the **Phase D contract** (the Node-panel buttons drive them). Next: **Phase D** (right-dock Node panel + filters + `DeviceDetails` with F3-gated Place/Move/Clear/Zoom + the unified Inspector) — the final Spec 4 phase.

## Post-MVP Pivot — Spec 4 (Nodes in 3D) Phase D — Node Panel, Device Inspector & Status Seam (2026-06-17)

The right-dock Node panel + the device Inspector complete the viewer. **Spec 4 is done → the 3D and permissions tracks have converged.**

- **Node panel (`775b4c5`, Task 1).** `viewport/ui/NodePanel.tsx`: the right-dock filterable, selection-synced device list (the monitoring surface + placement source) — `filterDevices(devices, nodeFilter, statusOf)` driven by text/placement/status/category controls (`setNodeFilter`); each row shows a category colour-chip · name · placed/unplaced · status dot, and a click `selectNode`s. 3 RTL units.
- **Device Inspector + zoom-to + right-dock (`73bb42b`, Task 2).** `viewport/ui/DeviceDetails.tsx`: the device branch of the unified Inspector — name/category/network/ip/status/position + **F3-gated** Place/Move/Clear (`canConfigure(access)` → OWNER/ADMIN only; MEMBER view-only) driving `beginPlace`/`clearPlacement`, and Zoom-to (`requestFocus`). `ui/Inspector.tsx` switches element ↔ device on `selection.kind`; `scene/ViewCommands.tsx` `requestFocus` now frames a device marker (`toViewport` + a 4m box) as well as an element mesh; `shell/ViewportHost.tsx` stacks `<NodePanel>` over `<Inspector>` in a right-dock column. 3 RTL units.

**Phase gate:** desktop **vitest 95 passed** (33 files; +6); client **vitest 6**; api **unit 484** (48 suites); `tsc -b --noEmit` clean (desktop + client); `electron-vite build` PASS (headless bundle); `nest build` clean. Verified headlessly in-sandbox.

### Spec 4 (Nodes in 3D) — COMPLETE ✅

All four phases built, TDD, on `feat/spec4-nodes-in-3d` (19 commits, stacked on `feat/spec3-viewport`): **A** foundations (`node-coords` xyz↔viewport, `filterDevices`/`canConfigure`/`NodeStatus`, the `?buildingPropertyId` server filter, client device methods, tagged `element|device` selection refactor) → **B** node layer (category-coded billboard `NodeLayer` + status ring, `useDeviceLoad` + realtime, marker-priority picking) → **C** placement (select-then-click surface raycast → `toModel` → `PATCH position`, optimistic + rollback, mid-air rejected, Esc) → **D** Node panel + filters + device Inspector (F3-gated Place/Move/Clear/Zoom) + unified Inspector switch. **The 3D track (Spec 1→2→3→4) and the permissions track (F2→F3) have converged.** Public interface for the **Monitoring spec**: `store.setNodeStatus(deviceId, NodeStatus)` lights up the existing marker rings / panel badges / status filter (v1 = `unknown`) — no other Spec 4 internal is in that contract; plus `node-coords` (`toViewport`/`toModel`) and the tagged `Selection` union. **Sandbox-verified headlessly** (vitest + tsc + both builds); the **api integration/e2e** (real `?buildingPropertyId` subtree query) and the **Playwright-Electron e2e** + live GPU/orbit/pick/place correctness run in the display/DB build env (spec §13). Branch pushed; **PR → master pending manual open** (gh/API token expired): `https://github.com/ztur211/nodescope/pull/new/feat/spec4-nodes-in-3d` (stacks on the unmerged spec2-phase-b → spec2-phase-c → spec3-viewport branches; merge those first).

## Post-MVP Pivot — Spec 5 (IFC Export) Phase A — IFC GUID + IFC2x3 Writer (2026-06-17)

The two pure, dependency-free IFC primitives — no DB, no HTTP, fully unit-tested. **Spec 5 (interop track) is independent (depends on Spec 1 only); branched off `master` as `feat/spec5-ifc-export`.** The service + F3-scoped streaming endpoint that feed real data are Phase B.

- **`toIfcGuid` (`d8c960d`, Task 1).** `apps/api/src/export/ifc-guid.ts`: deterministic 22-char IFC GlobalId over the IFC base-64 alphabet (`IFC_B64`). A real UUID is encoded **losslessly** (exact 128 bits → reversible, so re-exports are stable and coordination diffs work); any other seed is folded by a deterministic 128-bit FNV-1a hash. The device↔IFC key reused by Spec 6 (BCF). 4 unit tests (length/alphabet/determinism, 128-bit round-trip, distinct uuids, non-uuid seed). *Infra: `jest.unit.config` `testRegex` is now an array that also matches `export/__tests__/*.spec.ts` (the new module's pure specs carry no `.service`/`.validator`-style token).*
- **`buildNetworkIfc` (`437688c`, Task 2).** `apps/api/src/export/ifc2x3-writer.ts`: a pure `StepBuilder` (auto `#ids`) assembling a valid IFC2x3 STEP string — header (`FILE_SCHEMA(('IFC2X3'))`, CoordinationView), owner history, SI metre units + 3D context, the `IfcProject→IfcSite→IfcBuilding→IfcBuildingStorey` aggregation, a shared 0.2 m box marker, and per placed device an `IfcBuildingElementProxy` at `IfcCartesianPoint((x,y,z))` + `Pset_NodeScope` (Category/IP/MAC/Network/NodeScopeId) + one `IfcRelContainedInSpatialStructure`; zero devices → a valid empty stub. STEP-escapes device strings (`'`→`''`, backslash, control chars) for injection-safety (§9). `ExportDevice`/`BuildNetworkIfcInput` are Phase B's exact input. 2 unit tests (schema/proxy-count/coords/pset/containment/escaping; empty stub). **Plan-bug fixed:** the plan's strict-UUID `toIfcGuid` couldn't encode the writer's derived `${id}:pset`/`:site` seeds (BigInt parse error) — Task 1's hash fallback resolves it.

**Phase gate:** api **unit 487 passed** (50 suites; +6 export) + `nest build` clean. Pure unit tests — fully sandbox-verified. Deep IFC tool-import validity (Navisworks/Solibri) is a manual check with a sample export per spec §10 (deferred to the build env). `toIfcGuid` + `buildNetworkIfc` are the **Phase B contract** (the service gathers in-scope placed devices + network names and calls the writer; the controller streams it). Next: **Phase B** (`export.service` F2-subtree ∩ F3-scope device gather + `export.controller` `GET /v1/buildings/:propertyId/export/ifc` streaming download).

## Post-MVP Pivot — Spec 5 (IFC Export) Phase B — Export Service & Endpoint (2026-06-17)

The writer is fed real data and exposed as a download. **Spec 5 (IFC Export) is done.**

- **`ExportService` (`cab2074`, Task 1).** `apps/api/src/export/export.service.ts`: `getBuildingExport(member, buildingPropertyId)` validates the `propertyId` is an in-scope `BUILDING` Property (OWNER bypass; else `PermissionsService.inScope` — unknown OR out-of-scope → `PROP_001` 404, invisible-not-forbidden), resolves the F2 subtree ∩ F3 `scopeFilter` (null = OWNER), queries **placed** devices (`x/y/z` not null) with their `Network` name, maps to `ExportDevice[]`, and calls `buildNetworkIfc`; sanitized `{building}-network.ifc` filename. **4 unit tests** (mocked prisma/properties/permissions): OWNER full-subtree + the placed-only WHERE, unknown→404, out-of-scope→404, ADMIN subtree∩scope. *(The real-DB placement filter + scope are covered by the e2e in the build env.)*
- **`ExportController` + module + wiring (`1d88db6`, Task 2).** `GET /v1/buildings/:propertyId/export/ifc` (`@OrgMember`, `ParseUUIDPipe`) returns a **`StreamableFile`** (`Buffer.from(ifc)`, `application/x-step`, `attachment; filename=…`) — the codebase's file-download pattern (no `@Res()`/JSON-envelope). `ExportModule` (imports F2 `PropertiesModule` + F3 `PermissionsModule`; PrismaModule is `@Global`) wired into `AppModule`. A faithful **e2e** (`export.controller.e2e.ts`, modeled on the building-models e2e + an F3 member-property grant: OWNER 200 + headers + 1-proxy body, in-scope MEMBER 200, out-of-scope building → 404) is written for the **build env** (needs the DB stack).

**Phase gate:** api **unit 491 passed** (51 suites; +4 export.service) + `nest build` clean + `tsc -p tsconfig.jest.json` clean (all src + tests, incl. the e2e, type-check). The **e2e** (`test:e2e`) + manual IFC tool-import validity run in the build env (DB stack / IFC viewer). Doc: `docs/product-knowledge/ifc-export.md` (how an AEC consumer federates `*-network.ifc`).

### Spec 5 (IFC Export) — COMPLETE ✅

Both phases built, TDD, on `feat/spec5-ifc-export` (independent — depends on Spec 1 only; branched off `master`): **A** the pure dependency-free primitives (`toIfcGuid` deterministic IFC GlobalId + `buildNetworkIfc` IFC2x3 STEP writer — spatial tree, per-device `IfcBuildingElementProxy` at native coords + `Pset_NodeScope`, STEP-escaped, empty-stub) → **B** `ExportService` (in-scope placed-device gather, F2∩F3, 404 invisible-not-forbidden) + the `StreamableFile` download endpoint. Output contract = the federated IFC file (stable `toIfcGuid(device.id)` GlobalIds reused by Spec 6 BCF). **Sandbox-verified headlessly** (api unit 491, nest build, jest-tsconfig typecheck); the HTTP **e2e** + Navisworks/Solibri import are the build-env checks. **PR → master pending manual open** (gh token expired): `https://github.com/ztur211/nodescope/pull/new/feat/spec5-ifc-export` (independent of the Spec 2–4 desktop stack; merges cleanly onto master). **Interop track: Spec 5 done; only Spec 6 (BCF round-trip) remains** (depends on Spec 5 `toIfcGuid`).

## Post-MVP Pivot — Spec 6 (BCF Round-Trip) Phase A — .bcfzip codec (2026-06-17)

The pure BCF 2.1 `.bcfzip` reader/writer — the foundation the import/export services + viewport build on. Branch `feat/spec6-bcf`, **stacked on `feat/spec5-ifc-export`** (Spec 6 depends on Spec 5's `toIfcGuid` for device-linking in later phases).

- **`.bcfzip` codec (`1491ee5`, Phase A Task 2).** `apps/api/src/bcf/bcf-zip.ts`: pure `readBcfZip(buffer) → ParsedBcf` / `writeBcfZip(topics) → Buffer` over the BCF 2.1 archive (`jszip` container + `fast-xml-parser`, XXE-safe). Round-trips a topic's markup (title/type/status/priority/labels/author/dates/assignee/description) + comments (+ `viewpointGuid`) + the primary viewpoint (perspective **or** orthographic camera, `Components` selection + visibility/exceptions, snapshot PNG). `ParsedTopic`/`ParsedComment`/`ParsedViewpoint`/`BcfCamera`/`BcfComponents` are the cross-phase contract (Phases B/C/D consume them; the `camera`/`components` Json columns store the same shapes). New deps `jszip` + `fast-xml-parser`; `jest.unit.config` testRegex array extended for `bcf/__tests__/*.spec.ts`. **3 unit round-trip tests** (perspective lossless, orthographic + visibility-exception, archive structure: `bcf.version` + per-topic `markup.bcf`/`viewpoint.bcfv`).

**Phase gate:** api **unit 494 passed** (52 suites; +3 bcf) + `nest build` clean. Pure codec — fully sandbox-verified. **The rest of Spec 6 is deferred to the build env / merged base** and NOT built here: Phase A Task 1 = the Prisma models (`BcfTopic`/`BcfComment`/`BcfViewpoint`/`BcfTopicDevice`) + migration (needs the DB); Phase B = import service (DB upsert-by-guid + `StorageService` snapshots + `toIfcGuid` device-linking); Phase C = export + topic/comment CRUD + F3 `/v1/.../bcf/*` endpoints (DB/e2e); Phases D–E = the desktop Issues panel + `apply-viewpoint`/`capture-viewpoint` + realtime (need the merged Spec 4 desktop). Real-tool (Solibri/BIMcollab) `.bcfzip` interop is a manual check (spec §13). **PR pending manual open:** `https://github.com/ztur211/nodescope/pull/new/feat/spec6-bcf` (stacked on Spec 5 → merge Spec 5 first).

## Post-MVP Pivot — Spec 1 Follow-up: PATCH-position scope enforcement + realtime (2026-06-17)

Two gaps in the Spec 1 `PATCH /v1/devices/:id/position` endpoint, surfaced by the Spec 4 adversarial review (Spec 4's design assumes both are closed). Branch `fix/spec1-position-scope-and-realtime` off `master`.

- **Per-site scope enforcement.** The endpoint was `@OrgRoles('OWNER','ADMIN')`-only — an **out-of-scope ADMIN** could set the position of a device outside their assigned sites (a write scope-leak). `SpatialService.setPosition` now calls `PermissionsService.assertCanConfigure(member, device.propertyId)` after the org-wide device lookup (OWNER any; MEMBER → `ORG_003`; out-of-scope ADMIN → `PERM_001` 403, not 404). `@OrgRoles` retained (defence-in-depth). This is the server-side authority Spec 4 §8/§12 relies on (the desktop `canConfigure` gate is UX only).
...
## Post-MVP Pivot — Spec 7 (Device Health Monitoring — Server Pipeline + Embedded Prober) (2026-06-19)

The server-side monitoring pipeline that fills Spec 4's status-display seam: an agent-agnostic ingest seam, a current-status model + TimescaleDB history, state derivation, F3-scoped reads, a scope-filtered `v1:device:status` realtime event, an embedded ICMP/TCP prober (off by default), and a token-authed HTTP ingest endpoint. Built TDD on `feat/spec7-monitoring` (off `integration/full-stack`) and validated against a real native Postgres 16 + PostGIS + TimescaleDB 2.28 + Redis + MinIO stack.

- **Phase A — model + storage + ingest seam.** `DeviceStatus` (current state, 1:1 Device) + `MonitoringIngestToken` (per-org secret) Prisma models + `DeviceStatusState` enum. Two raw-SQL Timescale hypertables: **`MonitoringMetric`** (generic per-device health series) + `DeviceStatusEvent` (state timeline). Pure `deriveState` (UP/WARNING/DOWN, anti-flap `downThreshold`). `MonitoringRepository` (status upsert/read + hypertable `$executeRaw`/`time_bucket` `$queryRaw`). `IngestService` single write path: derive → upsert status → append `latency_ms`/`reachable` metrics → on a transition append `DeviceStatusEvent` + emit `v1:device:status` (via the `MONITORING_EMITTER` port); cross-org deviceId → `ORG_008`.
  - **Plan-vs-reality fix (caught by the real DB):** the generic metric table is named **`MonitoringMetric`, not `DeviceMetric`** — the legacy 2D app already owns a `DeviceMetric` hypertable (bandwidth/connectionQuality, managed by `TimescaleService`) with an incompatible schema. Distinct concern → own table. Also stripped Prisma's spurious `DROP INDEX device_location_idx` (raw-SQL-managed PostGIS GIST index) per the established migration convention.
- **Phase B — scoped reads + realtime + Spec 4 wiring.** `MonitoringGatewayEmitter` binds `MONITORING_EMITTER` to `ConflictResolutionService.emitScoped` (the F3-scoped fan-out — the plan's `RealtimeGateway.emitDeviceScoped` does not exist). `MonitoringService.getBuildingDeviceStatus` reuses Spec 4's `listDevicesForBuilding` (subtree ∩ F3 scope; missing rows → UNKNOWN); `getDeviceMetrics` is `inScope`-gated (OWNER bypass; out-of-scope → 404 invisible). `GET /v1/buildings/:propertyId/device-status` + `GET /v1/devices/:id/metrics`. `@nodescope/client.getBuildingDeviceStatus`/`getDeviceMetrics`. Desktop `useDeviceLoad` now seeds `nodeStatus` from the status read on building load and subscribes to `v1:device:status` → `setNodeStatus` (mapping via `statusFromState`), so Spec 4's marker rings / panel badges / status filter reflect **live** health with no redesign.
- **Phase C — embedded ICMP/TCP prober (off by default).** Dependency-free `probe.ts` (`tcpProbe` via `node:net`, `icmpProbe` via spawned `ping`, `probeDevice` = ICMP-then-TCP-fallback). `ProberService`: a concurrency-capped `setInterval` cycle over devices with an `ipAddress` → `reportStatusCheck(source:'prober')`, gated on `MONITORING_PROBER_ENABLED` (default **false** — cloud uses the Agent; self-host opts in). `runProbeCycle`/`mapLimit` pure + unit-tested.
