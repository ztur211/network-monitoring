...
- **C2 — `roleCodeOf` pure helper.** Maps each `DeviceCategory` enum value to its two-letter role abbreviation (e.g. `SWITCH → 'SW'`, `ACCESS_POINT → 'AP'`). Fully unit-tested; no I/O.
- **C3 — `naming-tokens` pure helper.** `renderTemplate(template, tokens)` substitutes `{site}`, `{building}`, `{floor}`, `{area}`, `{role}`, `{seq}` / `{seq:N}` tokens; collapses empty tokens and their surrounding separators; returns `null` for a fully empty result. `fillSeq(template, n)` renders a concrete sequence value. `hasSeqToken(template)` checks whether a collision-free sequence search is required. All helpers unit-tested.
- **C4 — `getAncestorChain` + `NameSuggestionService`.** `PropertiesRepository.getAncestorChain(propertyId)` runs a recursive CTE (`WITH RECURSIVE`) walking self → root, returns the ancestor chain ordered root-first. `NameSuggestionService.suggest(orgId, propertyId, category, roleCode?)` assembles the four ancestor tokens from the chain, calls `roleCodeOf` for the role token, renders the template, and when `{seq}` is present increments from 1 until `DevicesRepository.existsByNameCaseInsensitive` returns false — yielding a collision-free suggestion. Unit-tested end-to-end (mock repository, template rendering, seq scan, null path).
