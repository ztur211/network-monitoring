...
public class PropertiesContractTests
{
    ...
    public async Task ListProperties_returns_the_org_tree()
    {
        ...
        var response = await _api.GetAsync("v1/properties", org.OwnerCookie);
    ...
    public async Task GetProperty_returns_the_property_and_unknown_ids_are_404_PROP_001()
    {
        ...
        var found = await _api.GetAsync($"v1/properties/{siteId}", org.OwnerCookie);
        Assert.Equal(HttpStatusCode.OK, found.Status);
        Assert.Equal(siteId, found.Data.GetProperty("id").GetString());

        var missing = await _api.GetAsync($"v1/properties/{Guid.NewGuid()}", org.OwnerCookie);
    ...
    {
        var org = await _fixture.ProvisionOrgAsync();
        await InventoryScaffold.CreatePropertyAsync(_api, org.OwnerCookie, "SITE", name: "Duplicate Site");
    ...
    public async Task DeleteLeafProperty_succeeds_and_a_non_empty_parent_is_409_PROP_004()
    {
        ...
        var site = await InventoryScaffold.CreatePropertyAsync(_api, org.OwnerCookie, "SITE");
        var siteId = InventoryScaffold.RequireId(site);
        var building = await InventoryScaffold.CreatePropertyAsync(_api, org.OwnerCookie, "BUILDING", parentId: siteId);
        var buildingId = InventoryScaffold.RequireId(building);

        // The site now has a child, so it cannot be deleted.
        var blocked = await _api.DeleteAsync($"v1/properties/{siteId}", org.OwnerCookie);
        ...
        var deleted = await _api.DeleteAsync($"v1/properties/{buildingId}", org.OwnerCookie);
        Assert.Equal(HttpStatusCode.OK, deleted.Status);
        Assert.Equal(JsonValueKind.Null, deleted.Data.ValueKind);

        var gone = await _api.GetAsync($"v1/properties/{buildingId}", org.OwnerCookie);
