...
if [ "$DO_BACKEND" = 1 ]; then
  ...
  if [ ! -f "$ENV_FILE" ]; then
    if [ "$RESET" != 1 ] && docker volume inspect nodescope_pgdata >/dev/null 2>&1; then
      die "$ENV_FILE is missing while the NodeScope database volume exists; restore the file or rerun with --reset to replace the local appliance"
    fi
    log "generating $ENV_FILE (localhost appliance secrets) ..."
    {
      echo "# Generated by scripts/run-desktop.sh for local development. Keep with the Docker volumes; git-ignored."
      echo "PUBLIC_ORIGIN=${ORIGIN}"
      # Hex is safe in Compose environment files, shell tooling, and connection strings.
